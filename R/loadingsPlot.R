...
#' \code{loadingsPlot} uses the \code{arrayspecs} and \code{mshape} functions from the \code{geomorph} package (Adams and Otárola-Castillo 2013).
#'
...
#'
#' @importFrom geomorph arrayspecs mshape
#' @importFrom graphics layout
#' @importFrom grDevices rgb
#' @importFrom rgl mfrow3d plot3d
#' @export
loadingsPlot <- function(result, comp = 1, cex.2d = 2, cex.3d = 5) {
  ...
  if (class(result[[1]]) != "rgcca" & class(result[[1]]) != "prcomp") {
    stop("Object is not the expected format. Use analyseBlocks function to first analyse the data")
  }
  ...
  if (result$option == "pca") {
    ...
    if (k == 3) {
      ...
      for (i in 1:J) {
        ...
        block.loadings.bone <- matrix(block.loadings[pos.st[i]:pos.en[i], ], ncol = ncol(block.loadings), byrow = FALSE)
        block.loadings.PC <- matrix(block.loadings.bone[, comp], ncol = k, byrow = TRUE)
        ...
        plot3d(ref, add = FALSE, col = rgb(block.loadings.minmax, 0.5, 1 - block.loadings.minmax), aspect = "iso", axes = FALSE, xlab = "x", ylab = "y", zlab = "z", size = cex.3d)
      }
    }
    if (k == 2) {
      cl <- round(sqrt(J))
  ...
  if (result$option == "rcpca") {
    if (k == 3) {
      ...
      for (i in 1:J) {
        ...
        block.loadings.bone <- matrix(block.loadings[[J + 1]][pos.st[i]:pos.en[i], ], ncol = ncol(block.loadings[[J + 1]]), byrow = FALSE)
        block.loadings.PC <- matrix(block.loadings.bone[, comp], ncol = k, byrow = TRUE)
        block.loadings.len <- sqrt(((block.loadings.PC[, 1]^2) + (block.loadings.PC[, 3]^2)) + (block.loadings.PC[, 2]^2))
        block.loadings.minmax <- (block.loadings.len - min(block.loadings.len)) / (max(block.loadings.len) - min(block.loadings.len))
        plot3d(ref, add = FALSE, col = rgb(block.loadings.minmax, 0.5, 1 - block.loadings.minmax), aspect = "iso", axes = FALSE, xlab = "x", ylab = "y", zlab = "z", size = cex.3d)
