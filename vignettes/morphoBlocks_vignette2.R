...
block.list = combineBlocks(blocks = c(block1, block2, block3, block4, block5, block6, 
...
knitr::kable(table_2, caption = "Table 2. Correlation between multi-part samples after the data blocks in each sample were Procrustes-transformed and scaled with the normalised centroid size method described by Collyer et al. (2020).", col.names = gsub("[.]", " ", colnames(table_2)), align = "c", digits = 3) %>% 
  ...
  column_spec(2, color = "black", background = ifelse(table_2[,2] == 1, "#A0DFAF", ifelse(table_2[,2] == 0.998, "#99DBBB", ifelse(table_2[,2] >= 0.985, "#8FC7C6", "#A17FA9")))) %>% 
  column_spec(3, color = "black", background = ifelse(table_2[,3] == 1, "#A0DFAF", ifelse(table_2[,3] == 0.998, "#99DBBB", ifelse(table_2[,3] >= 0.985, "#8FC7C6", "#A17FA9")))) %>%
  column_spec(4, color = "black", background = ifelse(table_2[,4] == 1, "#A0DFAF", ifelse(table_2[,4] == 0.998, "#99DBBB", ifelse(table_2[,4] >= 0.985, "#8FC7C6", "#A17FA9")))) %>%
  column_spec(5, color = "black", background = ifelse(table_2[,5] == 1, "#A0DFAF", ifelse(table_2[,5] == 0.998, "#99DBBB", ifelse(table_2[,5] >= 0.985, "#8FC7C6", "#A17FA9")))) %>%
  column_spec(6, color = "black", background = ifelse(table_2[,6] == 1, "#A0DFAF", ifelse(table_2[,6] == 0.998, "#99DBBB", ifelse(table_2[,6] >= 0.985, "#8FC7C6", "#A17FA9")))) %>%
  column_spec(7, color = "black", background = ifelse(table_2[,7] == 1, "#A0DFAF", ifelse(table_2[,7] == 0.998, "#99DBBB", ifelse(table_2[,7] >= 0.985, "#8FC7C6", "#A17FA9")))) %>%
  column_spec(8, color = "black", background = ifelse(table_2[,8] == 1, "#A0DFAF", ifelse(table_2[,8] == 0.998, "#99DBBB", ifelse(table_2[,8] >= 0.985, "#8FC7C6", "#A17FA9")))) %>%
  column_spec(9, color = "black", background = ifelse(table_2[,9] == 1, "#A0DFAF", ifelse(table_2[,9] == 0.998, "#99DBBB", ifelse(table_2[,9] >= 0.985, "#8FC7C6", "#A17FA9")))) %>%
  column_spec(10, color = "black", background = ifelse(table_2[,10] == 1, "#A0DFAF", ifelse(table_2[,10] == 0.998, "#99DBBB", ifelse(table_2[,10] >= 0.985, "#8FC7C6", "#A17FA9")))) %>%
  column_spec(11, color = "black", background = ifelse(table_2[,11] == 1, "#A0DFAF", ifelse(table_2[,11] == 0.998, "#99DBBB", ifelse(table_2[,11] >= 0.985, "#8FC7C6", "#A17FA9"))))
...
# Figure 6
par(mfrow = c(1,2))
scoresPlot(result.rcpca, comp = c(1,2), plabels = 1:10, consensus.only = T)
scoresPlot(result.rcpca, comp = c(2,3), plabels = 1:10, consensus.only = T)
